...
`10 / 2`가 정수`5`가 아닌 부동 소수점`5.0`을 리턴하는 것에 주의하십시요. 이것은 의도된것입니다. Elixir에서는 `/`연산자는 항상 부동 소수점을 반환합니다. 만약 정수의 상과 나머지가 필요한 경우 함수`div`과`rem`를 호출하면 됩니다.:
...
Elixir는 2 진수, 8 진수, 16 진수 입력을 위해 짧은식을 지원합니다. 
...
Elixir는 변수의 형태를 조사하고 불리언 값을 반환하는 함수들을 제공하고 있습니다. 예를 들어, 함수 `is_boolean/1` 에서는 변수가 boolean 있는지 확인할 수 있습니다 :
...
> 참고 : Elixir 에서 함수들은 이름과 인자의 수(즉 항수)로 표시됩니다. 즉, `is_boolean/1`은 `is_boolean`라는 함수이름과 하나의 인수를 가지고 있음을 나타냅니다. `is_boolean/2`는 같은 이름이지만 항수가 다르기 때문에 다른 (존재하지 않는) 함수를 나타냅니다.
...
만약 인수가 정수, 부동 소수점 또는 그 중 하나 인 것을 확인하고 싶다면, 각각 `is_integer/1`, `is_float/1` 또는 `is_number/1` 을 사용할 수 있습니다.

...
일반적으로 모듈의 매크로를 사용하려고하는 경우를 제외하고는 사용하기 전에 모듈이 필요한 것은 아닙니다. 로드되지 않은 매크로를 호출하려고 하면 오류가 발생합니다. `alias` 지시어(directives)와 마찬가지로 `require`도 lexically scoped인 것에 주목하십시오. 매크로에 대해서는 이후의 장에서 다시 자세히 이야기 할것입니다.
